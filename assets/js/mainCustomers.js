$(document).ready(function () {
	...
	$('#tab_groups_customers').DataTable({
		ajax: "encodeGridGroupsCustomers.html",
		order: [
			[0, "asc"]
		],
		"columns": [
		...
		columnDefs: [

			{
				...
				render: function (data, full) {
					return '<button onclick = "takeIdForChangeName(' + data[0] + ')" type="button"  class="btn btn-info btn-fill" data-toggle="modal" data-target="#modal_update">Edition </button>';
				}
			},

			{
				"targets": 3,
				render: function (data, full) {
					return '<a id="btn_state" onclick="ajaxChangeStatus(' + data[0] + ',' + '\'' + "changeStatusGroupsCustomers.html" + '\'' + ',' + '\'' + "#tab_groups_customers" + '\'' + ')' + '"' + ' class="btn btn-danger btn-fill"><i class="fa fa-edit"></a>';
				}
			},
	...
	$('#tab_customers').DataTable({
		...
		columnDefs: [

			{
				"targets": 10,
				render: function (data, full) {
					return '<button onclick="editCustomersModal(' + data[0] + ')" type="button"  class="btn btn-info btn-fill" data-toggle="modal" data-target="#modal_update_customers">Edition </button>';
				}
			},
			{
				"targets": 11,
				render: function (data, full) {
					return '<a id="btn_state" onclick="ajaxChangeStatus(' + data[0] + ',' + '\'' + "changeStatusCustomers.html" + '\'' + ',' + '\'' + "#tab_customers" + '\'' + ')' + '"' + 'class="btn btn-danger btn-fill"><i class="fa fa-edit"></a>';
				}
			},
			{
				"targets": 12,
